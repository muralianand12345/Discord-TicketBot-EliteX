    ...
    async execute(interaction, client) {
        ...
        if (interaction.customId == "open-ticket-redm") {
            ...
            interaction.guild.channels.create(`redm-ticket-${interaction.user.username}`, {
                ...
                interaction.reply({
                    content: `RedM Ticket created! <#${c.id}>`,
                    ephemeral: true
                ...
                collector.on('collect', i => {
                    if (i.user.id === interaction.user.id) {
                        if (msg.deletable) {
                            msg.delete().then(async() => {
                                ...
                                const opened = await c.send({
                                    content: `**Your REDM Ticket Has Been Created!**`,
                                    embeds: [embed],
                                    components: [row]
                                });
                        ...
                        if (i.values[0] == 'Connect') {
                            c.edit({
                                parent: client.config.parentConnectRedm
                            });
                        };
