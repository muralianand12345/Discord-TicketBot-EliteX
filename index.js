...
const mongoFiles = fs.readdirSync('./database').filter(file => file.endsWith('.js'));

for (const file of mongoFiles) {
    const mongo = require(`./database/${file}`);
    client.on(mongo.name, (...args) => mongo.execute(...args, client));
};
